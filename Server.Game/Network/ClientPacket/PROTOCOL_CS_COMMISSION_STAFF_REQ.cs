        ...
        public override void Run()
        {
            try
            {
                ...
                for (int i = 0; i < PlayerIds.Count; i++)
                {
                    ...
                    if (member != null && member.ClanId == clan.Id && member.ClanAccess == 3 && ComDiv.UpdateDB("accounts", "clan_access", 2, "player_id", member.PlayerId))
                    {
                ...
                Client.SendPacket(new PROTOCOL_CS_COMMISSION_STAFF_ACK(result));
            }
            catch (Exception ex)
            {
