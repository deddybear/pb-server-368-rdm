...
namespace Server.Match.Network.Actions.Event
{
    public class DropWeapon
    {
        public static DropWeaponInfo ReadInfo(ActionModel Action, SyncClientPacket C, bool GenLog)
        {
            ...
            if (GenLog)
            {
                CLogger.Print($"Slot: {Action.Slot}; Drop WeaponId: {Info.WeaponId}; Flag: {Info.WeaponFlag}; Extensions: {Info.Extensions}", LoggerType.Warning);
                CLogger.Print($"Ammo Prin: {Info.AmmoPrin}; Ammo Dual: {Info.AmmoDual}; Ammo Total: {Info.AmmoTotal}", LoggerType.Warning);
            }
            return Info;
        }
        public static void WriteInfo(SyncServerPacket S, DropWeaponInfo Info, int Count)
        {
            S.WriteD(Info.Unk2);
            S.WriteC(Info.Unk3);
            ...
            S.WriteC((byte)(Info.Extensions + Count));
